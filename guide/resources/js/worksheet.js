const parseListData = (() => {
   ...
   */
  getCateList('/guide/json/_map.json')
    .then(dataOrigin => {
 ...
 */
 async function getCateList(url) {
  ...
  cateResponse.forEach((arr) => {
    ...
    let urlLink = '';
    for (let i in arr) {
      if (i == 0) cateKey = arr[i].directory.replace('/html/', ''), urlLink = arr[i].directory;
...
const setHtmlTbody = (urlLink, _origin, _index) => {
  ...
  if(_html.id) {
    if(_html.link) {
      ...
      urlLink = (_html.id.indexOf('CMM') > -1 ? '/html/CMM/' : urlLink+'/') + _html.id + '.html';
