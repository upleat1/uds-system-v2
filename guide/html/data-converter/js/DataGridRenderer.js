...
var DataGridRenderer = {
    ...
    html: function(dataGrid, headerNames, headerTypes, indent, newLine) {
        ...
        // input value [get array]
        var getClass = document.querySelector(".class-name").value;
        var arrClass = getClass.split(",");
        ...
        for (var i = 0; i < numRows; i++) {
            ...
            for (var j = 0; j < numColumns; j++) {
                if (arrClass[j] == "undefined" || arrClass[j] == "") {
                    outputText += indent + indent + indent + '<td>';
                    console.log(arrClass[j]);
                } else {
                    outputText += indent + indent + indent + '<td class="' + arrClass[j] + '">';
                    console.log(arrClass, arrClass[j]);
                }
                outputText += row[j]
    ...
    xml: function(dataGrid, headerNames, headerTypes, indent, newLine) {
        ...
        //begin render loop
        outputText = '<?xml version="1.0" encoding="euc-kr"?>' + newLine;
